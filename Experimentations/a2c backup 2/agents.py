...
class ActorCritic(object):
    ...
    def compute_loss(self, action_probs, returns, values):
        ...
        action0_entropy = -tf.math.log(action0_probs)
        action1_entropy = -tf.math.log(action1_probs)
        action2_entropy = -tf.math.log(action2_probs)
        action3_entropy = -tf.math.log(action3_probs)
        action4_entropy = -tf.math.log(action4_probs)
        action5_entropy = -tf.math.log(action5_probs)
        action6_entropy = -tf.math.log(action6_probs)
        action7_entropy = -tf.math.log(action7_probs)

        actor0_loss = tf.math.reduce_mean(action0_entropy * advantage)
        actor1_loss = tf.math.reduce_mean(action1_entropy * advantage)
        actor2_loss = tf.math.reduce_mean(action2_entropy * advantage)
        actor3_loss = tf.math.reduce_mean(action3_entropy * advantage)
        actor4_loss = tf.math.reduce_mean(action4_entropy * advantage)
        actor5_loss = tf.math.reduce_mean(action5_entropy * advantage)
        actor6_loss = tf.math.reduce_mean(action6_entropy * advantage)
        actor7_loss = tf.math.reduce_mean(action7_entropy * advantage)
